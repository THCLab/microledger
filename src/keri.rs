...
use keri::{processor::validator::EventValidator, event::sections::seal::EventSeal, event_message::signature::Nontransferable, database::SledEventDatabase};

use crate::{signature::KeriSignature, verifier::Verifier, microledger::Result};
...
impl Verifier for KeriVerifier {
    ...
    fn verify(&self, data: &[u8], s: Vec<Self::Signature>) -> Result<bool> {
        Ok(s.into_iter().all(|sig| self.0.verify(data, &sig.into()).is_ok()))
    }
}
    ...
    fn into(self) -> keri::event_message::signature::Signature {
        ...
        match self {
            KeriSignature::Transferable(id, sn, dig, sigs) => Signature::Transferable(
                SignerData::EventSeal(EventSeal { prefix: id, sn, event_digest: dig }), sigs.into_iter().map(|s| s.into()).collect()
            ),
            KeriSignature::Nontransferable(bp, ssp) => Signature::NonTransferable(Nontransferable::Couplet(vec![(bp, ssp)]))
        }
    }
}
...
pub mod test {
    ...
    use keri::{prefix::{BasicPrefix, SelfSigningPrefix}, keys::PublicKey, database::SledEventDatabase, processor::basic_processor::BasicProcessor};
    use rand::rngs::OsRng;
    use sai::derivation::SelfAddressing;
    use tempfile::Builder;

    use crate::{block::{Block, SignedBlock}, digital_fingerprint::DigitalFingerprint, microledger::MicroLedger, controlling_identifier::ControllingIdentifier, seals::Seal, seal_bundle::{SealBundle, SealData}, signature::KeriSignature, Encode, keri::KeriVerifier};
    ...
    fn test_microledger_traversing() {
        ...
        let deserialize_microledger  =
            MicroLedger::<KeriSignature, _>::new_from_cesr(serialized_microledger.as_bytes(), validator).unwrap();
